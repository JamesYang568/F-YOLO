...
class BaseYOLONeck(BaseModule, metaclass=ABCMeta):
    ...
    # 主要分为 Reduce 层， UpSample 层，TopDown 层，DownSample 层，BottomUP 层以及输出outLayer
    @abstractmethod
    ...
    def forward(self, inputs: List[torch.Tensor]) -> tuple:
        ...
        # reduce layers
        reduce_outs = []
        for idx in range(len(self.in_channels)):  # C3、C4、C5 完成降维
            reduce_outs.append(self.reduce_layers[idx](inputs[idx]))  # 最深层的reduce，其他的直接在conv中完成降维

        # top-down path
        inner_outs = [reduce_outs[-1]]  # 这里还保存了中间连接的输出（非常重要！！）
        for idx in range(len(self.in_channels) - 1, 0, -1):  # 从深层（高层）向低层
            feat_high = inner_outs[0]  # 每一次都是在向down-top横向之后进入上采样操作
            feat_low = reduce_outs[idx - 1]
            upsample_feat = self.upsample_layers[len(self.in_channels) - 1 - idx](feat_high)
            # 执行拼接操作
            if self.upsample_feats_cat_first:  # 谁在前
                top_down_layer_inputs = torch.cat([upsample_feat, feat_low], 1)
            else:
                top_down_layer_inputs = torch.cat([feat_low, upsample_feat], 1)
            inner_out = self.top_down_layers[len(self.in_channels) - 1 - idx](top_down_layer_inputs)
            inner_outs.insert(0, inner_out)  # 插在最前面

        # bottom-up path
        outs = [inner_outs[0]]  # top-down的最后一个直接输出
        for idx in range(len(self.in_channels) - 1):
            feat_low = outs[-1]  # 上一层传来的（放在最后）
            feat_high = inner_outs[idx + 1]  # 当前
            downsample_feat = self.downsample_layers[idx](feat_low)
            out = self.bottom_up_layers[idx](
                torch.cat([downsample_feat, feat_high], 1))
            outs.append(out)  # 就是按照特征图由大变小（越来越深）的顺序来的
