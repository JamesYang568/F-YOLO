...
class PPYOLOEHeadModule(BaseModule):
    ...
    def forward_single(self, x: Tensor, cls_stem: nn.ModuleList,
        ...
        cls_logit = cls_pred(cls_stem(x, avg_feat) + x)  # 分类支，首先经过分类的stem完成注意力机制，之后和shortcut相加过卷积
        bbox_dist_preds = reg_pred(reg_stem(x, avg_feat))

        # TODO: Test whether use matmul instead of conv can speed up training.

        bbox_dist_preds = bbox_dist_preds.reshape(
            [-1, 4, self.reg_max + 1, hw]).permute(0, 2, 3, 1)  # 展成一个个框
...
class PPYOLOEHead(YOLOv6Head):
    ...
    def loss_by_feat(
        ...
        # If the shape does not equal, generate new one  最开始一定会走进这个，这里取巧，先使用后定义
        if current_featmap_sizes != self.featmap_sizes_train:
            self.featmap_sizes_train = current_featmap_sizes

            self.mlvl_priors_with_stride = self.prior_generator.grid_priors(  # (coord_x, coord_y, stride_w, stride_h)
            ...
            self.num_level_priors = [len(n) for n in self.mlvl_priors_with_stride]  # 每一个level的点数
            self.level_stride_tensor = [n[..., [2]] for n in self.mlvl_priors_with_stride]

            self.flatten_priors_train = torch.cat(self.mlvl_priors_with_stride, dim=0)  # 拼接
            self.stride_tensor = torch.cat(self.level_stride_tensor, dim=0)  # 拼接
        ...
        # pred info  [b,80,w,h] | [b,1,w*h,4] | [b,reg_max+1,w*h,4] --> [b,w*h,80] | [b,w*h,4*1] | [b,w*h,4*(reg_max+1)]
        flatten_cls_preds = [
        ...
        flatten_pred_dists = [
            bbox_pred_org.permute(0, 2, 3, 1).reshape(num_imgs, -1, (self.head_module.reg_max + 1) * 4)
            for bbox_pred_org in bbox_dist_preds
        ]

        flatten_cls_preds = torch.cat(flatten_cls_preds, dim=1)  # 不同level的加在一起
        pred_scores = torch.sigmoid(flatten_cls_preds)

        flatten_dist_preds = torch.cat(flatten_pred_dists, dim=1)  # 不同level的加在一起 [b,wh,17*4]

        # tmp = [self.bbox_coder.decode(  # todo  有很大改进空间，1、可以zip。2、使用下标,而非保存每个level
        #     self.mlvl_priors_with_stride[i][..., :2],
        #     flatten_pred_bboxes[i],
        #     self.level_stride_tensor[i][..., 0]
        # ) for i in range(len(flatten_pred_bboxes))
        # ]
        # refined_pred_bboxes = self.deform_sampling(tmp, offset)  # [[b,wh,4]]
        # refined_pred_bboxes = torch.cat(refined_pred_bboxes, dim=1)  # 不同level的拼在一起 [b,wh,4]

        flatten_pred_bboxes = torch.cat(flatten_pred_bboxes, dim=1)  # 不同level的拼在一起 [b,wh,4]
        flatten_pred_bboxes = self.bbox_coder.decode(  # ltrb转bbox  [b,wh,4]
        ...
        if num_pos > 0:
            ...
            pred_dist_pos = torch.masked_select(
                flatten_dist_preds,  # 使用概率作为筛选
                dist_mask
            ).reshape([-1, 4, self.head_module.reg_max + 1])
    ...
    # todo remove删除这部分代码！！！
    def deform_sampling(self, feat: Sequence[Tensor], offset: Sequence[Tensor]) -> List[Tensor]:
        """Sampling the feature x according to offset.

        Args:
            feat (Sequence[Tensor]): Feature [[b,hw,4] len for level]
            offset (Sequence[Tensor]): Spatial offset for feature sampling list [[b,8,h,w] len for level]
        """
        # it is an equivalent implementation of bilinear interpolation
        y = []
        for i in range(len(self.featmap_sizes_train)):
            h, w = self.featmap_sizes_train[i]
            b, hw, c = feat[i].shape
            weight = feat[i].new_ones(c, 1, 1, 1)
            bbox = feat[i].reshape([-1, h, w, 4]).permute(0, 3, 1, 2).contiguous()  # shift to [b,4,h,w]
            y.append(deform_conv2d(bbox, offset[i], weight, 1, 0, 1, c, c).reshape([b, 4, hw]).permute(0, 2, 1))
        return y
