...
---
    ...
    Args:
        arch_setting (list): Architecture of BaseBackbone.  从配置文件中读取
    ...
    def __init__(self,
        ...
        for idx, setting in enumerate(arch_setting):  # build之后的连接已经做好了
            stage = []
    ...
    def make_stage_plugins(self, plugins, stage_idx, setting):
        ...
        Args:
            ...
            stage_idx (int): Index of stage to build  注意这里实际上名字起的不好，plugin在哪个stage使用是要看plugins提供的参数的
                If stages is missing, the plugin would be applied to all
                stages.  按照上面的例子，stage_idx实际上是控制一个stage中plugin的顺序的
            setting (list): The architecture setting of a stage layer.
        ...
        for plugin in plugins:
            ...
            if stages is None or stages[stage_idx]:
                name, layer = build_plugin_layer(
                    plugin['cfg'], in_channels=in_channels)  # 注意这里在backbone进行了优化，传入plugin时自动带有in_channels
