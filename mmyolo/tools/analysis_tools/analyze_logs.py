...
def plot_curve(log_dicts, args):
    ...
    for i, log_dict in enumerate(log_dicts):
        ...
        if args.start_epoch is None:
            args.start_epoch = epochs[0]
        if args.end_epoch is None:
            args.end_epoch = epochs[-1]
        assert epochs[0] <= args.start_epoch
        assert epochs[-1] >= args.end_epoch
        epochs = epochs[args.start_epoch - 1:args.end_epoch]
        for j, metric in enumerate(metrics):
            ...
            if 'mAP' in metric:
                ...
                # if i == 0:  # 计算new loss的时候，对不齐，可以这样修改对齐 todo
                #     ys = ys / 2
                # if i == 1:
                #     xs = xs[:len(xs) // 2]
                #     ys = ys[:len(ys) // 2]
...
def add_plot_parser(subparsers):  # changed!! please check keys, start_epoch, end_epoch
    ...
    parser_plt.add_argument(
        ...
        type=str,
        nargs='+',
        default=['bbox_mAP'],  # 'bbox_mAP_s', 'bbox_mAP'
    ...
    parser_plt.add_argument('--title', type=str, help='title of figure', default='AP')
    parser_plt.add_argument(
        '--legend',
        type=str,
        nargs='+',
        default=['ours', 'baseline',  # 'bbox_mAP',  # 'bbox_mAP_s',
                 ],
