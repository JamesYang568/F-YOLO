    ...
    data_preprocessor=dict(
        ...
        batch_augments=[
            dict(
                ...
                # 注意，因为pipeline是在数据集层次上进行的数据增强，因此对于batch内的增强就需要processor，相当于collect_fn
...
val_evaluator = dict(
    ...
    proposal_nums=(100, 1, 10),  # 所有的yolo都是这个
...
optim_wrapper = dict(
    ...
    optimizer=dict(
        ...
        nesterov=True),
    paramwise_cfg=dict(norm_decay_mult=0., bias_decay_mult=0.)  # param_dicts 这里没有进行不同层不同学习率
    #   paramwise_cfg=dict(custom_keys={'backbone': dict(lr_mult=0.1, decay_mult=1.0)}))
)

# learning rate
param_scheduler = [  # todo 学习这个做法
    dict(
...
train_cfg = dict(
    ...
    val_interval=save_epoch_intervals / 2,
